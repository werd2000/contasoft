...
require_once DIRMODULOS . 'Clientes/Modelo/ClientesModelo.php';
require_once 'App/LibQ/Input.php';

/**
 *  Clase Controladora del Modulo Clientes
...
/**
    ...
    private $_paramBotonNuevo = array(
        'href' => 'index.php?option=clientes&sub=agregar',
     ...
     * @var type Array
     */
    private $_paramBotonVolver = array('href'=>'index.php?option=clientes');
    ...
    private $_paramBotonLista = array(
        'href' => 'index.php?option=clientes&sub=listar',
    ...
    {
        parent::__construct();
        $this->_vista->addScriptPath(DIRMODULOS . 'Clientes/Vista');
        require_once DIRMODULOS . 'Clientes/Modelo/ClientesModelo.php';
        $this->_modelo = new ClientesModelo();
    }
    
    /**
     * Metodo que lleva al menu de los clientes
     * @return void
     */
    public function index ()
    {
        $this->_layout->content = $this->_vista->render('ClientesVista.php');
    ...
    {
        require_once DIRMODULOS . 'Clientes/Forms/CargaClientes.php';
        require_once LibQ . 'LibQ_BarraHerramientas.php';
//        require_once LibQ . 'MyFechaHora.php';
        $this->_form = new Form_CargaClientes($this->_varForm);
        ...
        $this->_layout->content = $this->_vista->render('AgregarClienteVista.php');
        // render final layout
        echo $this->_layout->render();
    }
    ...
    public function editar ($arg)
    {
        ...
        $datos_array = $this->_modelo->listadoClientes(Array('id' , 'razon_social'));
        foreach ($datos_array as $clienteBuscado) {
            $clientes[] = array($clienteBuscado->id => $clienteBuscado->razon_social);
        }
        $this->_form = new Form_CargaClientes($cuentas, $clientes, $this->_varForm);
        $this->_vista->form = $this->_form->mostrar();
        if ($_POST) {
    ...
    public function listar ($arg='')
    {
        ...
        $grilla->setUrl(LIVESITE . '/index.php?option=clientes&sub=jsonListarClientes');
        ...
        $grilla->setColModel(array(
            ...
            array('name'=>'Cliente', 'index'=>'razon_social', 'width'=>160),
        ...
        $grilla->setActionOnDblClickRow('/index.php?option=clientes&sub=editar&id=');
        $bh = new LibQ_BarraHerramientas($this->_vista);
        $bh->addBoton('Nuevo', $this->_paramBotonNuevo);
        $bh->addBoton('Filtrar', $this->_paramBotonFiltrar);
        if (is_array($arg)) {
            $filtroBoton = '&' . implode("&", $arg) . '&lista=clientes';
        } else {
            $filtroBoton = '&lista=gastos';
        }
        $bh->addBoton('Exportar', array('href' => 'index.php?option=clientes&sub=exportar' . $filtroBoton,
        ));
        $bh->addBoton('Volver', $this->_paramBotonVolver);
        $this->_vista->LibQ_BarraHerramientas = $bh->render();
        $this->_vista->grid = $grilla->incluirJs();
        $this->_layout->content = $this->_vista->render('ListadoClienteVista.php');
        echo $this->_layout->render();
    }
    
    public function jsonListarClientes($arg='')
    {
        ...
        $result = $this->_modelo->listadoClientes($inicio, $orden, '' );
        $count = count($result);
        $responce->page = $pag;
    ...
    /**
     * 
     * Lista los resumenClientesCliente
     * @param string $arg
     */
    public function resumenClientesCliente ($arg='')
    {
        require_once LibQ . 'JQGrid.php';
        require_once LibQ . 'LibQ_BarraHerramientas.php';
        $grilla = new JQGrid('grilla');
        $grilla->setTitulo('GASTOS POR PROVEEDOR');
        $grilla->setUrl(LIVESITE . '/index.php?option=gastos&sub=jsonListarClientes');
        $grilla->setColNames(array(
            "'id'"=>"'Id'",
            "'cuenta'"=>"'Cuenta'",
            "'razon_social'"=>"'Cliente'",
        ...
        $grilla->setColModel(array(
            ...
            array('name'=>'Cliente', 'index'=>'razon_social', 'width'=>130),
    ...
    public function resumenClientesMensual ($arg='')
    {
        ...
        $fuenteDatos = $this->_modelo->resumenClientesMensual($inicio, $orden, $filtro);
    ...
    /**
     * 
     * Lista los resumenClientesClienteAnio
     * @param string $arg
     */
    public function resumenClientesClienteAnio ($arg='')
    {
        require_once 'App/LibQ/Grilla.php';
        require_once 'App/vistas/html/LibQ_BarraHerramientas.php';
        $campoRecibido = '';
        ...
        $fuenteDatos = $this->_modelo->resumenClientesClienteAnio($inicio, $orden, $filtro);
        $grilla = new Grilla($fuenteDatos);
        $grilla->setTotalPaginas(ceil($this->_modelo->getCantidadRegistros($filtro)/$this->_config->get('limiteGrilla')));
        $grilla->setPagina($pag);
        ...
        if ($valorRecibido != 'valor' && $valorRecibido != '') {
            ...
            switch ($campoRecibido) {
                ...
                case 'razon_social':
                    ...
                    $datos_array = $this->_modelo->listadoClientes(Array('id', 'razon_social'));
                    foreach ($datos_array as $clienteBuscado) {
                        $clientes[$clienteBuscado->razon_social] = $clienteBuscado->id;
                    }
                    $valor = $clientes[$valorRecibido];
                    break;
                default:
...
//                $this->_redirect('index.php?option=gastos&sub=listar');
    ...
    private function _controlar_nro_factura($nro_factura='', $cliente='', $comprobante='', $tipo_comprobante='') 
    {
        ...
        $gastoBuscado = $this->_modelo->buscarCliente($consulta);
