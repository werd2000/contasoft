 ...
 * 
 */
class ImpuestosControlador extends ControladorBase
{
    /**
...
/**
    ...
    private $_paramBotonNuevo = array(
        'href' => 'index.php?option=impuestos&sub=agregar',
     ...
     * @var type Array
     */
    private $_paramBotonVolver = array('href'=>'index.php?option=impuestos');
    ...
    private $_paramBotonLista = array(
        'href' => 'index.php?option=impuestos&sub=listar',
    ...
    {
        parent::__construct();
        $this->_vista->addScriptPath(DIRMODULOS . 'Impuestos/Vista');
        require_once DIRMODULOS . 'Impuestos/Modelo/ImpuestosModelo.php';
        $this->_modelo = new ImpuestosModelo();
    }
    ...
    {
        $this->_layout->content = $this->_vista->render('ImpuestosVista.php');
    ...
    {
        require_once DIRMODULOS . 'Impuestos/Forms/CargaImpuestos.php';
        require_once LibQ . 'LibQ_BarraHerramientas.php';
        require_once LibQ . 'MyFechaHora.php';
        $this->_form = new Form_CargaImpuestos( $this->_varForm);
        ...
        $this->_layout->content = $this->_vista->render('AgregarImpuestoVista.php');
        // render final layout
        echo $this->_layout->render();
    }
    ...
    public function editar ($arg)
    {
        ...
        require_once LibQ . 'MyFechaHora.php';
        $where = implode(',', $arg);
        $impuestoBuscado = $this->_modelo->buscarImpuesto($where);
        if (is_object($impuestoBuscado)){
            $this->_varForm['id'] = $impuestoBuscado->id;
            $this->_varForm['impuesto'] = $impuestoBuscado->impuesto;
            $this->_varForm['caracter'] = $impuestoBuscado->caracter;
            $this->_varForm['tipo_vencimiento'] = $impuestoBuscado->tipo_vencimiento;
            $this->_varForm['observaciones'] = $impuestoBuscado->observaciones;
            $this->_varForm['eliminado'] = $impuestoBuscado->eliminado;
        } else {
            $this->_varForm['id'] = '0';
            $this->_varForm['impuesto'] = '';
            $this->_varForm['caracter'] = '';
            $this->_varForm['tipo_vencimiento'] = '';
...
//                $values['fecha_comprobante']=implode('/', array_reverse(explode('/', $values['fecha_comprobante'])));
    ...
    public function listar ($arg='')
    {
        ...
        $grilla->setUrl(LIVESITE . '/index.php?option=impuestos&sub=jsonListarImpuestos');
        $grilla->setColNames(array(
            "'id'"=>"'Id'",
            "'impuesto'"=>"'Impuesto'",
            "'caracter'"=>"'Caracter'",
            "'tipo_vencimiento'"=>"'Tipo Vencimiento'",
        ...
        $grilla->setColModel(array(
            ...
            array('name'=>'Caracter', 'index'=>'caracter', 'width'=>130),
            array('name' => 'Tipo Vencimiento', 'index' => 'tipo_vencimiento', 'width' => 130),
        ...
        $this->_layout->content = $this->_vista->render('ListadoImpuestosVista.php');
        echo $this->_layout->render();
    }
    
    public function jsonListarImpuestos($arg='')
    {
//        print_r($arg);
        ...
        $result = $this->_modelo->listadoImpuestos($inicio, $orden, '' );
        $count = count($result);
        $responce->page = $pag;
    ...
    public function ultimosImpuestos()
    {
//        setlocale(LC_MONETARY, 'es_AR');
        $retorno = '<table>';
        $i = 0;
        $listaImpuestos = $this->_modelo->listadoImpuestos(0, 'id DESC', '');
        foreach ($listaImpuestos as $impuesto) {
...
//             money_format ('%i',$impuesto['total'])
        ...
        /**
     * 
     * Lista los resumenImpuestosProveedor
     * @param string $arg
     */
    public function resumenImpuestosProveedor ($arg='')
    {
        require_once LibQ . 'JQGrid.php';
        require_once LibQ . 'LibQ_BarraHerramientas.php';
        $grilla = new JQGrid('grilla');
    ...
    public function resumenImpuestosMensual ($arg='')
    {
        require_once LibQ . 'JQGrid.php';
        require_once LibQ . 'LibQ_BarraHerramientas.php';
        $grilla = new JQGrid('grilla');
        $grilla->setTitulo('GASTOS POR PROVEEDOR');
        $grilla->setUrl(LIVESITE . '/index.php?option=impuestos&sub=jsonListarImpuestosMensual');
        $grilla->setColNames(array(
            "'id'"=>"'Id'",
            "'cuenta'"=>"'Cuenta'",
            "'razon_social'"=>"'Proveedor'",            
    ...
    /**
     * 
     * Lista Resumen de Impuestos Por Anio
     * @param string $arg
     */
    public function jsonListarImpuestosMensual ($arg='')
    {
        /** Me fijo si hay argumentos */
        if (isset($arg)) {
            /** Me fijo si existe el argumento page */
        ...
        $todos = $this->_modelo->getCantidadRegistros();
        $total_pages = ceil($todos / 30);
        $result = $this->_modelo->resumenImpuestosMensual($inicio, $orden, '' );
//        print_r($result);
        $count = count($result);
        $responce->page = $pag;
        $responce->total = $total_pages;
    ...
    public function graficoImpuestosMensuales(){
        $anio = date('Y');
        $mes1 = date('m')-1;
        $mes2 = $mes1 - 1;
        $mes3 = $mes2 - 1;
        $encabezado = "['Mes','Importe $']";
        $dato1 = $this->_modelo->totalImpuestosMensual($anio, $mes1);
        $dato2 = $this->_modelo->totalImpuestosMensual($anio, $mes2);
        $dato3 = $this->_modelo->totalImpuestosMensual($anio, $mes3);
...
//        $retorno .= "['2007',  1030]";
        ...
        $grafico = new ChartGoogle('Impuestos Mensuales');
